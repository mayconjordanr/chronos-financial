...
import { CardService, CreateCardData, UpdateCardData } from './cards.service';
import {
...
export class CardController {
   ...
   * Create a new card
   */
  async createCard(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
      ...
      const validation = CreateCardRequestDTO.safeParse(request.body);
      ...
      return reply.code(201).send({
        ...
        data: {
          ...
          // CRITICAL: Never include sensitive data in response
        }
      });
    } catch (error) {
   ...
   */
  async getCards(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
        return reply.code(401).send({
          success: false,
      ...
      const filtersValidation = CardFiltersDTO.safeParse(request.query);
      ...
      const paginationValidation = PaginationDTO.safeParse(request.query);
   ...
   */
  async getCard(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
        return reply.code(401).send({
          success: false,
      ...
      const paramsValidation = CardParamsDTO.safeParse(request.params);
   ...
   */
  async updateCard(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
        return reply.code(401).send({
          success: false,
      ...
      // Validate request body
      const bodyValidation = UpdateCardRequestDTO.safeParse(request.body);
      ...
      const result = await this.cardService.updateCard(
        cardId,
        data,
        userId,
        tenantId
   ...
   * Delete/lock card
   */
  async deleteCard(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
        return reply.code(401).send({
          success: false,
   ...
   * Activate card
   */
  async activateCard(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
        return reply.code(401).send({
          success: false,
   ...
   * Deactivate card
   */
  async deactivateCard(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
        return reply.code(401).send({
          success: false,
   ...
   */
  async getCardTransactions(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
        return reply.code(401).send({
          success: false,
   ...
   * Get card statistics
   */
  async getCardStatistics(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
        return reply.code(401).send({
          success: false,
   ...
   * Bulk activate cards
   */
  async bulkActivateCards(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
        return reply.code(401).send({
          success: false,
      ...
      // Validate request body
      const validation = BulkActivateCardsDTO.safeParse(request.body);
      if (!validation.success) {
        return reply.code(400).send({
          success: false,
          message: 'Invalid request data',
   ...
   */
  async validateCardTransaction(request: AuthenticatedRequest, reply: FastifyReply) {
    try {
      if (!request.user) {
        return reply.code(401).send({
          success: false,
      ...
      // Simple validation for amount (should be more robust in production)
      ...
      const result = await this.cardService.validateCardTransaction(
        cardId,
        amount,
        userId,
        tenantId
